...
char * trim(char *str)
{
	...
	{
		rp = (char *)malloc(strlen(str) - 1);
		strncpy(rp, str, strlen(str) - 1);
		rp[strlen(str) - 1] = 'D\0';
		str= (char *)malloc(strlen(str) - 1);
		strcpy(str, rp);
	}
...
char *transferHDSerial( char *HDSerial  )
{
	char *pRet, *pValue ;
	int nLen = strlen( HDSerial) ;
	pValue = (char*)malloc( nLen + 1 ) ;
	pRet = pValue ;
	memset( pRet, 0, nLen + 1 ) ;
	int i = 1 , j = 10 ;
	while( *HDSerial)
	{
		if( ( *HDSerial >= 48 ) && ( *HDSerial <= 57 ))
		{ 
			*HDSerial = *HDSerial + i ;
			if( *HDSerial > 57 )
		 {
...
{	
	...
	// return m_buffer;
	return transferHDSerial( trim(m_buffer) ) ;
}
